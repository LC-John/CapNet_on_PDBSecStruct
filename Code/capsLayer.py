...
def routing(input, b_IJ):
    ...
    if cfg.dataset == "rna":
        W = tf.get_variable('Weight', shape=(1, 1824, 2, 8, 16), dtype=tf.float32,
                            initializer=tf.random_normal_initializer(stddev=cfg.stddev))
    elif cfg.dataset == "pdb":
        W = tf.get_variable('Weight', shape=(1, 1568, 3, 8, 16), dtype=tf.float32,
                            initializer=tf.random_normal_initializer(stddev=cfg.stddev))
    elif cfg.dataset == "ori-pdb":
        W = tf.get_variable('Weight', shape=(1, 1568, 8, 8, 16), dtype=tf.float32,
                            initializer=tf.random_normal_initializer(stddev=cfg.stddev))
    else:
    ...
    if cfg.dataset == "rna":
        ...
        input = tf.tile(input, [1, 1, 3, 1, 1])
        W = tf.tile(W, [cfg.batch_size, 1, 1, 1, 1])
    elif cfg.dataset == "ori-pdb":
        input = tf.tile(input, [1, 1, 8, 1, 1])
        W = tf.tile(W, [cfg.batch_size, 1, 1, 1, 1])
    else:
    ...
    for r_iter in range(cfg.iter_routing):
        with tf.variable_scope('iter_' + str(r_iter)):
            ...
            if r_iter == cfg.iter_routing - 1:
                ...
                if cfg.dataset == "rna":
                    v_J_tiled = tf.tile(v_J, [1, 1824, 1, 1, 1])
